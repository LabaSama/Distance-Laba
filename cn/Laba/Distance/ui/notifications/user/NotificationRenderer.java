package cn.Laba.Distance.ui.notifications.user;
...
import cn.Laba.Distance.util.render.RenderingUtil;
...
public class NotificationRenderer implements INotificationRenderer {
   ...
   public void draw(List<Notification> notifications) {
      ...
      for (Iterator<?> var5 = notifications.iterator(); var5.hasNext(); y += 32.0F) {
         ...
         float headerWidth = Client.FontLoaders.Chinese20.getStringWidth(not.getHeader() + (not.getType().equals(Notifications.Type.MUSIC)? "( \u2022¦Ø\u2022)\u266A~":""));
         ...
         Client.FontLoaders.Chinese20.drawStringWithShadow(not.getHeader() + (not.getType().equals(Notifications.Type.MUSIC)? ((System.currentTimeMillis() / 100L) % 30 == 0 ?"( - ¦Ø -)" : "( \u2022¦Ø\u2022)") + "\u266A~":""), x + 2 + 24.0F, not.translate.getY() + 4 + 5F, -1);
